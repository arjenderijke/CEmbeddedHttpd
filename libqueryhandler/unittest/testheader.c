...
void testGetHeader(void)
{
    ...
    curl_easy_setopt(c, CURLOPT_HTTPHEADER, responseHeaders);
    ...
    curl_easy_getinfo(c, CURLINFO_CONTENT_TYPE, &ct);
    CU_ASSERT_STRING_EQUAL(ct, ACCEPT_JSON);

    CU_ASSERT(rh.header_count == 7);
    CU_ASSERT_TRUE(rh.has_date);
    CU_ASSERT(rh.content_length == 22);
...
void testGetXml(void)
{
    ...
    curl_easy_getinfo(c, CURLINFO_CONTENT_TYPE, &ct);
    CU_ASSERT_STRING_EQUAL(ct, ACCEPT_XML);
...
void testGetHversion(void)
{
    ...
    CU_ASSERT_TRUE(rh.has_date);
    CU_ASSERT(rh.content_length == 24);
    ...
    CU_ASSERT(cbc.pos == 24);
    CU_ASSERT(excess_found == 0);
}

void testGetHeader10(void)
{
    struct MHD_Daemon *daemon;
    CURL *c;
    char buf[2048];
...
void testGetCsv(void)
{
    ...
    curl_easy_getinfo(c, CURLINFO_CONTENT_TYPE, &ct);
    CU_ASSERT_STRING_EQUAL(ct, ACCEPT_CSV);
...
void testHeadHeader(void)
{
	...
	curl_easy_getinfo(c, CURLINFO_RESPONSE_CODE, &resp);
	CU_ASSERT(resp == MHD_HTTP_METHOD_NOT_ALLOWED);
...
int main()
{
    ...
    pSuite = CU_add_suite("Urls post requests header", initCurl, cleanCurl);
    if (pSuite == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test get header", testGetHeader) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test get xml", testGetXml) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test get header version", testGetHversion) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test get header 1.0", testGetHeader10) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test get csv", testGetCsv) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test post header", testPostHeader) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test head header", testHeadHeader) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test put header", testPutHeader) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test delete header", testDeleteHeader) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }
