...
int main()
{
    ...
    if (CU_add_test(pSuite, "test http 1.0", testHttp10) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }

    if (CU_add_test(pSuite, "test http 1.0 root", testHttp10Root) == NULL) {
	CU_cleanup_registry();
	return CU_get_error();
    }
    /*
